<template>
  <div class="job-view">
    <v-card outlined class="pa-5">
      ...
      <div class="footer">
        <v-btn color="grey darken-1" @click="verCandidaturas(id)" dark tittle="Ver Candidatos">Ver Candidatos</v-btn>
        <v-btn v-if="status != 'Encerrado'" color="primary" @click="editarVaga(id)" title="Editar Vaga">Editar</v-btn>
        <v-btn v-else disabled color="primary" @click="editarVaga(id)" title="Editar Vaga (desabilitado)">Editar</v-btn>
        <v-btn color="error" v-if="status!='Encerrado'" @click="fecharVaga(id)" tittle="Fechar Vaga">Fechar</v-btn>
      </div>
    </v-card>
  </div>
</template>
